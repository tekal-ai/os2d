...
class LITWDataset(Dataset):
    def __init__(self, reference_path, class_path, annotations_path, box_coder):
    ...
    def __getitem__(self, idx):
        ...
        reference_image = Image.open(os.path.join(self.reference_path, imageid)).convert("RGB")
        ...
        for name in names:
            ...
            images = [Image.open(f'{self.class_path}/{name}/{image}').convert("RGBA") \
                          for image in os.listdir(f'{self.class_path}/{name}') if image[-4:] == '.jpg' or image[-4:] == '.png' or image[-5:] == '.jpeg']
        ...
        return reference_image_th, class_th, loc_targets, class_targets, idx_df["classid"].tolist(), class_size, \
...
class OS2DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        idx_df = self.annotations_df[self.annotations_df["imageid"] == idx]
        reference_image = Image.open(os.path.join(self.reference_path, str(idx) + ".jpg"))
        class_images = [Image.open(os.path.join(self.class_path, str(class_id) + ".jpg")) for class_id in idx_df["classid"].unique()]
        class_ids = idx_df["classid"].tolist()
        bbox = torch.Tensor(np.array(idx_df[['lx', 'ty', 'rx', 'by']]))
