...
from train_litw import get_litw_batch
...
cfg.init.model = "alternating-models/checkpoint_iter_17500.pth"#"trained-models/checkpoint_iter_780000.pth"
cfg.is_cuda = True
...
#cfg.train.optim.optim_method = "adam"
...
cfg.output.save_iter = 2500
cfg.output.path = 'alternating-4ratio-models'
cfg.eval.iter = 5000
...
def trainval_loop2():
    ...
    # start training
    i_epoch = 0
    i_batch = 17500 % max_index  # to start a new epoch at the first iteration
    i_litw_batch = 17500 % 7236
    t0 = time.time()
    ts = []
    for i_iter in range(17500,cfg.train.optim.max_iter):
        t1 = time.time()
        ts.append(t1-t0)
        t0 = t1
        if i_iter % 25 == 0: 
        ...
        if i_iter % 4 == 0:
            try:
                i_litw_batch += 1
                batch_litw_data = get_litw_batch(i_litw_batch)
            except:
                i_litw_batch += 1
                continue
        ...
        # train on one batch
        train_one_batch(batch_data, net, cfg, criterion, optimizer)
        if i_iter % 4 == 0:
            train_one_batch(batch_litw_data, net, cfg, criterion, optimizer)
