...
class SyntheticAugmentationsDataset(Dataset):
    ...
    def sample_xy_coordinates(self, img_size, object_size, max_occlusion, step, num_samples):
        #x_min_fraction = (step // int(np.sqrt(num_samples))) / np.sqrt(num_samples) 
        #x_max_fraction = ((step + int(np.sqrt(num_samples))) // int(np.sqrt(num_samples))) / np.sqrt(num_samples)
        #y_min_fraction = (step % int(np.sqrt(num_samples))) / np.sqrt(num_samples)
        #y_max_fraction = ((step + 1) % int(np.sqrt(num_samples))) / np.sqrt(num_samples)
        #print(step, num_samples, x_min_fraction, x_max_fraction, y_min_fraction, y_max_fraction)
        #x_fraction = random.uniform(x_min_fraction, x_max_fraction)
        #y_fraction = random.uniform(y_min_fraction, y_max_fraction)
        #print(x_fraction, y_fraction)

        #print(step, x_fraction, y_fraction)
        x_min = -object_size[0] * max_occlusion
        x_max = img_size[0] - (1 - max_occlusion) * object_size[0]

        y_min = -object_size[1] * max_occlusion
        y_max = img_size[1] - (1 - max_occlusion) * object_size[1]

        #x = int(x_min + x_fraction * (x_max - x_min))
        #y = int(y_min + y_fraction * (y_max - y_min))

        x = int(random.uniform(x_min, x_max))
        y = int(random.uniform(y_min, y_max))
        return x, y

    def _get_mean_lab_color_of_png_image(self, img):
        img_arr = np.array(img).reshape(-1, 4)

        rgb = img_arr[:, :3]
        alpha = img_arr[:, 3]

        avg_color_rgb = np.sum(rgb * alpha[:, np.newaxis], axis=0) / np.sum(alpha) 
        avg_color_rgb = sRGBColor(avg_color_rgb[0], avg_color_rgb[1], avg_color_rgb[2])
        avg_color_lab = convert_color(avg_color_rgb, LabColor) 
        
        return avg_color_lab

    def _sample_object_placements(self, img, object_to_paste, angle, max_occlusion=0.15, num_samples=16):
        sampled_placements = []
        rad = np.radians(angle)
        object_w, object_h = object_to_paste.size
        img_w, img_h = img.size
        
        avg_object_color_lab = self._get_mean_lab_color_of_png_image(object_to_paste)
        
        xy_coords = []
        delta_e = []
        for i in range(num_samples):
            cur_x, cur_y = self.sample_xy_coordinates((img_w, img_h), (object_w, object_h), max_occlusion, i, num_samples)
            
            x_left = max(0, cur_x)
            x_right = min(cur_x + np.abs(object_w * np.cos(rad)) + np.abs(object_h * np.sin(rad)), img_w)
            y_top = max(0, cur_y)
            y_bottom = min(cur_y + np.abs(object_h * np.cos(rad)) + np.abs(object_w * np.sin(rad)), img_h)
            
            cropped_img = img.crop((x_left, y_top, x_right, y_bottom))
            avg_color_lab = self._get_mean_lab_color_of_png_image(cropped_img)
            
            xy_coords.append([cur_x, cur_y])
            delta_e.append(delta_e_cie2000(avg_object_color_lab, avg_color_lab))
            
        max_contrast_idx = np.argmax(delta_e)
        
        return xy_coords[max_contrast_idx]

    def _sample_angle(self, min_angle=-30, max_angle=30, p=0.5):
    ...
    def __getitem__(self, idx):
        ...
        min_dim_ref = min(w_ref, h_ref)
        max_dim_class = max(w_class, h_class)

        h_new = int(h_class * ratio * min_dim_ref / max_dim_class)
        w_new = int(w_class * ratio * min_dim_ref / max_dim_class)
        ...
        x_corner, y_corner = self._sample_object_placements(reference_image, class_img_to_paste, angle)
